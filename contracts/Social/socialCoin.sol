...
contract socialCoin is Context, IERC20, IERC20Metadata, Ownable{
    ...
    constructor(string memory name_ , string memory symbol_, address payable myWallet, uint taxPercent) {
        _name = name_;
        _symbol = symbol_;
        _mint(msg.sender, 100*10**decimals()); // add the correct amount before deploying!
    ...
    /* this function returns all the wallets who are tax exempt, like the liquidity
    provider waller, the ICO contract and the sharer rewards contract */ 

    function returnNoTaxWallets() external view returns (address [] memory){
        return allNoTaxWallets; 
    }

    /* views the tax amount to make sure its the right amount (in %)  */
    /* refer to the _transfer function to view how it is used, lines 160 to 194 */
    function viewTax() external view returns(uint){
    return x;
    }

    /* views the address recieving the tax amount */

    function viewStorageWallet() external view returns(address){
    return storageWallet;
    }

    /* allows flexibility for further applications that could require a change of address */

    function changeStorageWallet(address payable myNewWallet) external onlyOwner {
        storageWallet = myNewWallet;
    }

    /*  allows flexibility for further applications that could require a change of percentage */

    function changeTaxPercent(uint newTaxPercent) external onlyOwner {
        x = newTaxPercent;
    }

    /* allows us add tax exempt addresses */
